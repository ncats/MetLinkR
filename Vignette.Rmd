...
## Organizing inputs and building identifier manifest

Next we need to organize the input data files and build a csv manifest to describe the metadata provided for our metabolite entities in our inputs. The example manifest for this vignette is as follows:
...
1. The first column, "FileNames" provides the pathname for each of the five supplied input datasets. All input data files should be stored in the directory indicated in their pathname - in this example, the "example_data" directory. 
...
2. `n_cores`: the number of cores for parallelization
3. `mapping_library_format`: one of the main outputs of a metLinkR analysis is the mapping library, which contains the associations with metabolite species from each input file to a consensus name generated by RefMet. This parameter specifies if this library should be returned in "long" or "wide" or "both" formats.
4. `remove_parentheses_for_synonym_search`: We found a common issue with parsing common names was that some platforms would include redundant or secondary information in parentheses following a primary ID. This optional filter removes this information after attempting a primary match.
5. `use_metabolon_parsers`: removes special characters such as “*” and “(1)” or “(2)” when dealing with ambiguous or duplicate features from the Metabolon platform, respectively
6. `majority_vote`: When multiple IDs are supplied for a metabolite, either (1) use the most common RefMet output among options, or (2) prioritize options in the following order: HMDB > KEGG > LIPIDMAPS > Chebi > Common Name > PubChem
